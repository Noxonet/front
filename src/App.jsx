        ...
        <div className="mt-6">
          ...
          <div className="border border-gray-400 rounded-lg mt-6 min-h-[200px] sm:min-h-[280px]">
            <div className="flex items-center justify-between py-3 sm:py-4 px-6 sm:px-10 border-b border-gray-400">
              ...
              <Link to="#" className="text-black text-sm font-semibold hover:underline">View All</Link>
            </div>
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem('token', data.token);
        // Unlock signup bonus
        await fetch('http://localhost:5000/api/bonuses/unlock-signup', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        Swal.fire({
          ...
          text: `Your referral code: ${data.referralCode}. You have unlocked a 5 USDT signup bonus!`,
          confirmButtonColor: 'white',
          confirmButtonText: '<span class="text-black">Ok</span>',
        });
        navigate('/tasks');
