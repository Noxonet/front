...
function SignupPage({ setIsLoggedIn, setBalance, updateBalance }) {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // Update inviter's referral count if referral code is provided
      if (referralCode) {
      ...
      // Update logged-in state and balance
      setIsLoggedIn(true);
      setBalance(0);
      await updateBalance(); // Sync with database
      ...
      // Wait briefly to ensure database sync
      setTimeout(() => {
        navigate('/assets');
      }, 1000);
    } catch (error) {
