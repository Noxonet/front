...
function DepositPage({ method }) {
  ...
  const [walletAddress,setWalletAddress] = useState('bnb1ys2g3ssskz4t6yqs238nj0pe8ncsjwq6rhjrxg');
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      let newBalance = (userData.balance || 0) + parseFloat(countUsd);
      let bonusMessage = '';
      if (!userData.hasFirstDepositBonus) {
        newBalance += 10; // Add $10 first deposit bonus
        bonusMessage = '\nReceived $10 First Deposit Bonus';
        await fetchWithErrorHandling('PATCH', `users/${auth.currentUser.uid}`, {
          balance: newBalance,
          hasFirstDepositBonus: true,
        });
      } else {
        await fetchWithErrorHandling('PATCH', `users/${auth.currentUser.uid}`, {
          balance: newBalance,
      ...
      Swal.fire({
        ...
        text: `New balance: ${updatedData.balance.toFixed(2)} USDT${bonusMessage}`,
        confirmButtonColor: '#1f2937',
        confirmButtonText: 'OK',
        customClass: {
          popup: 'bg-white shadow-2xl rounded-lg animate-fade-in',
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 bg-gray-50 rounded-lg !text-black border border-gray-200 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition-colors"
              />
            </div>
          </div>
          <button
